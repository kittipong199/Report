...
(() => {
  ...
  OOP.AvevaApplication = class AvevaApplication {
    constructor(legacy = window.AVEVA) {
      ...
      this.filters = new OOP.FilterService({ repository: this.repository, state: this.state, config: this.config, utils: this.utils, translate: legacy.t });
      ...
      this.charts = new OOP.ChartRenderer(legacy);
      ...
      this.events = new OOP.EventController({ loader: this.loader, state: this.state, loadingView: this.loadingView, filters: this.filters, dashboard: this.dashboard, table: this.table, charts: this.charts, config: this.config });
      ...
      legacy.fillTableFilters = this.table.fillFilters.bind(this.table);
      ...
      legacy.fillFilters = this.filters.fill.bind(this.filters);
      legacy.getFilters = () => this.filters.values;
      legacy.validateDateRange = this.filters.validateDateRange.bind(this.filters);
      legacy.syncYearMonthFilter = this.filters.syncYearMonth.bind(this.filters);
      legacy.defaultYear = this.filters.defaultYear.bind(this.filters);
